  ...
  const getNavItems = () => {
    if (user?.role === 'Citizen') {
      return [
        ...
        { to: '/citizen/report/new', label: 'Report Damage', icon: FileText },
        { to: '/citizen/reports', label: 'Track Reports', icon: CalendarDays },
      ];
    } else if (user?.role === 'Municipal Officer') {
      return [
        { to: '/officer/dashboard', label: 'Incoming Requests', icon: LayoutDashboard },
        { to: '/officer/reports', label: 'Completed Reports', icon: CheckCircle2 },
      ];
    }
    return [];
  };
